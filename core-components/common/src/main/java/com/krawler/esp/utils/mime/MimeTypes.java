 ...
 * enables to retrieves a content-type from a specified file extension, or from
 * a magic character sequence (or both).
 * 
...
public final class MimeTypes {
	...
	/**
	 * My registered instances There is one instance associated for each
	 * specified file while calling the {@link #get(String)} method. Key is the
	 * specified file path in the {@link #get(String)} method. Value is the
	...
	/**
	 * Returns an array of matching MimeTypes from the specified name (many
	 * MimeTypes can have the same registered extensions).
	 */
	private MimeType[] getMimeTypes(String name) {
		...
		return (mimeTypes != null) ? (MimeType[]) mimeTypes
				.toArray(new MimeType[mimeTypes.size()]) : null;
	}
