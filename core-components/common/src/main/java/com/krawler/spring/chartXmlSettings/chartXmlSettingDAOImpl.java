...
public class chartXmlSettingDAOImpl implements chartXmlSettingDAO {
    ...
    public KwlReturnObject getBarChartXML(HashMap<String, Object> requestParams) throws ServiceException {
        ...
        Boolean show_legend=requestParams.containsKey("show_legend") ?(Boolean) requestParams.get("show_legend"):false;
        ...
        try {
            ...
            Element d_sep = doc.createElement("decimals_separator");
            ...
            Element integer_only = doc.createElement("integers_only");
            integer_only.appendChild(doc.createTextNode("1"));
            values_val.appendChild(integer_only);
...
//            if(!StringUtil.isNullOrEmpty(doubleBar)||!StringUtil.isNullOrEmpty(title1)) {
//                enabled.appendChild(doc.createTextNode("1"));//set 1 if you want to shoew legend
//            }else {
                enabled.appendChild(doc.createTextNode(show_legend?"1":"0"));
  //          }
            legend.appendChild(enabled);

            Element border_color = doc.createElement("border_color");
            border_color.appendChild(doc.createTextNode("FFFFFF"));
            ...
            align.appendChild(doc.createTextNode("left"));
            legend.appendChild(align);

            Element key = doc.createElement("key");
            legend.appendChild(key);
            Element key_border_color = doc.createElement("border_color");
            key_border_color.appendChild(doc.createTextNode("000000"));
            ...
            if(!StringUtil.isNullOrEmpty(title1)) {
            ...
            xmlString = sw.toString().substring(38, sw.toString().length()); //Remove <?xml ?> tag for charts
            ll.add(xmlString);
    ...
    public KwlReturnObject getPieChartXML(HashMap<String, Object> requestParams) throws ServiceException {
        ...
        String title = requestParams.containsKey("title") ? requestParams.get("title").toString() : "";        
        try {
            String group_percent = requestParams.containsKey("group_percent") ? requestParams.get("group_percent").toString() : "5";
            Boolean hidehide_balloon_value=requestParams.containsKey("hide_balloon_value") ?(Boolean) requestParams.get("hide_balloon_value"):false;
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
            DocumentBuilder db = dbf.newDocumentBuilder();
            Document doc = db.newDocument();
            ...
            show.appendChild(doc.createTextNode("{title} "+(hidehide_balloon_value?"":"({value})")+"  {percents}%    <br>{description} "));
            balloon.appendChild(show);

            Element legend = doc.createElement("legend");
            root.appendChild(legend);
            Element enabled = doc.createElement("enabled");            
            enabled.appendChild(doc.createTextNode("0"));  //set 1 if you want to shoew legend
            legend.appendChild(enabled);
            Element align = doc.createElement("align");
            align.appendChild(doc.createTextNode("left"));
            legend.appendChild(align);
            ...
            Element grp_percentage = doc.createElement("percent");
            grp_percentage.appendChild(doc.createTextNode(group_percent));
            grp.appendChild(grp_percentage);
            ...
            pie_height.appendChild(doc.createTextNode("0"));//for 3D look
            pie.appendChild(pie_height);
            Element pie_angle = doc.createElement("angle");
            pie_angle.appendChild(doc.createTextNode("0"));
            ...
            data_labels_show.appendChild(doc.createTextNode("{title} "+(hidehide_balloon_value?": {description}":"({value})"+"")));
            data_labels.appendChild(data_labels_show);
            Element data_labels_radius = doc.createElement("radius");
            data_labels_radius.appendChild(doc.createTextNode("50%"));
            data_labels.appendChild(data_labels_radius);
            ...
            Element hide_labels_percent = doc.createElement("hide_labels_percent");
            hide_labels_percent.appendChild(doc.createTextNode("2"));
            data_labels.appendChild(hide_labels_percent);
            Element labels = doc.createElement("labels");
            root.appendChild(labels);
