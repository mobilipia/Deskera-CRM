 ...
 * @author Karthik
 */
public class permissionHandlerController extends MultiActionController {
    ...
    public ModelAndView setPermissions(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String permAuditMsg = (String) kmsg.getEntityList().get(1);
            auditTrailDAOObj.insertAuditLog(ADMIN_Permission,
                    "Permissions updated for " + userLogin.getUser().getFirstName() + " " + userLogin.getUser().getLastName() + " - " + permAuditMsg,
                    request, userid);
            ...
            if (userid.equals(currentUserId)) {
                ...
                if(jobj.has("Perm")) {
                    // remove all permissions set in server session
                    JSONObject permJSON = jobj.getJSONObject("Perm");
                    Iterator pernameite = permJSON.keys();
                    while(pernameite.hasNext()) {
                        String permname = pernameite.next().toString();
                        httpsession.removeAttribute(permname);
                    }
                }
            ...
            jobj.put("msg", "Permissions assigned successfully.");
