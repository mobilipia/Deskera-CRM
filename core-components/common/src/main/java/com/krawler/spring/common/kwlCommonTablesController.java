...
//    }
//     private String getFormattedDateWithTimeZone(Date curDate, String javaForm,HttpServletRequest request) throws SessionExpiredException {
//        SimpleDateFormat sdf = new SimpleDateFormat(javaForm);
...
//                obj.put("name", getFormattedDateWithTimeZone(new Date(), dateFormat.getJavaForm(),request)); // need to format with time format id as well
                obj.put("name", kwlCommonTablesDAOObj.getUserDateFormatter(dateFormat.getFormatID(), timeFormatId,timeZoneDiff).format(new Date()));
