...
var EV = Wtf.lib.Event;

Wtf.ux.ManagedIFrame = function(){
    ...
    if(el && el.dom && el.dom.tagName == 'IFRAME'){
            ...
            Wtf.get(Wtf.DomHelper.append(config.autoCreate.parent||document.body,
                Wtf.apply({tag:'iframe', src:(Wtf.isIE&&Wtf.isSecure)?Wtf.SSL_SECURE_URL:''},config.autoCreate))):null;
    }
    ...
    this.addEvents({
         ...
         * @param {Wtf.ux.ManagedIFrame} this
         * @param {Wtf.Event}
    ...
    if(config.listeners){
        ...
        Wtf.ux.ManagedIFrame.superclass.constructor.call(this);
    }

    Wtf.apply(el,this);  // apply this class interface ( pseudo Decorator )
    ...
    if(document.addEventListener){  //for Gecko and Opera and any who might support it later
       Wtf.EventManager.on(window,"DOMFrameContentLoaded", el.dom[el._eventName]);
    }

    var um = el.updateManager=new Wtf.UpdateManager(el,true);
    ...
    return Wtf.ux.ManagedIFrame.Manager.register(el);

};

var MIM = Wtf.ux.ManagedIFrame.Manager = function(){
  var frames = {};
  return {
    ...
    hideShims : function(){
        ...
        Wtf.select('.'+this.shimCls,true).removeClass(this.shimCls+'-on');
    ...
    showShims : function(){
       if(!this.shimApplied){
          ...
          Wtf.select('.'+this.shimCls,true).addClass(this.shimCls+'-on');
       }
 ...
 //Provide an Wtf.Element interface to frame document elements
 ...
 Wtf.apply(MIM.El.prototype,Wtf.Element.prototype);



 Wtf.extend(Wtf.ux.ManagedIFrame , Wtf.util.Observable,
    {
    ...
    setSrc : function(url, discardUrl, callback){
          var reset = Wtf.isIE&&Wtf.isSecure?Wtf.SSL_SECURE_URL:'';
    ...
    reset     : function(src, callback){
          this.setSrc(src || (Wtf.isIE&&Wtf.isSecure?Wtf.SSL_SECURE_URL:''),true,callback);
    ...
    update : function(content,loadScripts,callback){
        ...
        content = Wtf.DomHelper.markup(content||'');
    ...
    ,fly : function(el, named){
        ...
        if(!MIM._flyweights[named]){
            MIM._flyweights[named] = new Wtf.Element.Flyweight();
        }
    ...
    /**
     * Creates a {@link Wtf.CompositeElement} for child nodes based on the passed CSS selector (the selector should not contain an id).
     * @param {String} selector The CSS selector
     * @param {Boolean} unique (optional) True to create a unique Wtf.Element for each child (defaults to false, which creates a single shared flyweight object)
     * @return {CompositeElement/CompositeElementLite} The composite element
     */
    ,select : function(selector, unique){
        var d;
        return (d = this.getDocument())?Wtf.Element.select(selector, unique, d):null;
     }
    ...
    ,query : function(selector){
        var d;
        return (d = this.getDocument())?Wtf.DomQuery.select(selector, d):null;
     }


    /**
     * Returns the current HTML document object as an {@link Wtf.Element}.
     * @return Wtf.Element The document
    */
    ,getDoc  : function(){
    ...
    ,_renderHook : function(){
        ...
        this._hooked = false;
        try{
           if(this.writeScript('(function(){(window.hostMIF = parent.Wtf.get("'+
                                this.dom.id+
                                '"))._windowContext='+
                                (Wtf.isIE?'window':'{eval:function(s){return eval(s);}}')+
                                ';})();')
                ){
    ...
    sendMessage : function (message,tag,origin){
         ...
         if(this.disableMessaging !== true && (win = this.getWindow())){
              ...
              tag || (tag= message.tag || '');
              tag = tag.toLowerCase();
              message = Wtf.applyIf(message.data?message:{data:message},
                                 {type   :Wtf.isEmpty(tag)?'message':'message:'+tag
                                 ,domain :origin || document.domain
                                 ,uri    : document.documentURI
                                 ,source : window
                                 ,tag    :tag || null
    ...
    */
    ...
    destroy:function(){
        ...
        if(this.dom){
             ...
             if(document.addEventListener){ //Gecko/Opera
                Wtf.EventManager.un(window,"DOMFrameContentLoaded", this.dom[this._eventName]);
               }
             this.dom[this._eventName]=null;
             Wtf.ux.ManagedIFrame.Manager.deRegister(this);
             this._windowContext = null;
             //IE Iframe cleanup
             if(Wtf.isIE && this.dom.src){
        ...
        Wtf.apply(this.loadMask,{masker :null ,maskEl : null});
    ...
    ,writeScript  : function(block, attributes) {
        attributes = Wtf.apply({},attributes||{},{type :"text/javascript",text:block});
     ...
     * @param {String/Object} name of the function or
                              function map object: {name:'encodeHTML',fn:Wtf.util.Format.htmlEncode}
     * @param {Boolean} useDOM - if true inserts the fn into a dynamic script tag,
    ...
    ,showMask: function(msg,msgCls,forced){
          ...
          if((lmask = this.loadMask) && (!lmask.disabled|| forced)){
               ...
               lmask.masker || (lmask.masker = Wtf.get(lmask.maskEl||this.dom.parentNode||this.wrap({tag:'div',style:{position:'relative'}})));
               lmask._vis = true;
...
var CSSInterface = function(hostDocument){
 ...
 /*
  * @class Wtf.ux.ManagedIFramePanel
  ...
  *
 */
 Wtf.ux.ManagedIframePanel = Wtf.extend(Wtf.Panel, {
    ...
    * Custom CSS styles to be applied to the ux.ManagedIframe element in the format expected by {@link Wtf.Element#applyStyles}
    * (defaults to CSS Rule {overflow:'auto'}).
    */
    frameStyle  : {overflow:'auto'},
    frameConfig : null,
    hideMode    : !Wtf.isIE?'nosize':'display',
    shimCls     : Wtf.ux.ManagedIFrame.Manager.shimCls,
    shimUrl     : null,
    loadMask    : false,
    animCollapse: Wtf.isIE,
    autoScroll  : false,
    closable    : true, /* set True by default in the event a site times-out while loadMasked */
    ctype       : "Wtf.ux.ManagedIframePanel",
    showLoadIndicator : false,

    /**
    *@cfg {String/Object} unsupportedText Text (or Wtf.DOMHelper config) to display within the rendered iframe tag to indicate the frame is not supported
    */
   ...
   ,initComponent : function(){
        ...
        this.bodyCfg ||
                    ...
                    ,children:[
                        Wtf.apply(
                          Wtf.apply({
                            ...
                            ,unsup , Wtf.isIE&&Wtf.isSecure?{src:Wtf.SSL_SECURE_URL}:false )
                            //the shimming agent
                            ,{tag:'img', src:this.shimUrl||Wtf.BLANK_IMAGE_URL , cls: this.shimCls }
                         ]
                    }
                  ]
           });
         ...
         Wtf.ux.ManagedIframePanel.superclass.initComponent.call(this);
    ...
    doLayout   :  function(){
        ...
        if(this.fitToParent && !this.ownerCt){
            var pos = this.getPosition(), size = (Wtf.get(this.fitToParent)|| this.getEl().parent()).getViewSize();
            this.setSize(size.width - pos[0], size.height - pos[1]);
        }
        Wtf.ux.ManagedIframePanel.superclass.doLayout.apply(this,arguments);
    ...
    beforeDestroy : function(){
        ...
        if(this.rendered){
             ...
             Wtf.each(['iframe','shim','header','topToolbar','bottomToolbar','footer','loadMask','body','bwrap'],
                function(elName){
                  if(this[elName]){
                    if(typeof this[elName].destroy == 'function'){
                         this[elName].destroy();
                    } else { Wtf.destroy(this[elName]); }
        ...
        Wtf.ux.ManagedIframePanel.superclass.beforeDestroy.call(this);
    },
    onDestroy : function(){
    ...
    onRender : function(ct, position){
        Wtf.ux.ManagedIframePanel.superclass.onRender.call(this, ct, position);
        ...
        if(this.iframe = this.body.child('iframe.x-managed-iframe')){
            ...
            if(this.loadMask){
                this.loadMask = Wtf.apply({disabled     :false
                                          ,maskEl       :this.body
                                          ,hideOnReady  :true}
                                          ,this.loadMask);
             }

            if(this.iframe = new Wtf.ux.ManagedIFrame(this.iframe, {
            ...
            while(ownerCt){
                ...
                ownerCt.on('afterlayout',function(container,layout){
                        var MIM = Wtf.ux.ManagedIFrame.Manager,st=false;
                        Wtf.each(['north','south','east','west'],function(region){
                            var reg;
        ...
        this.shim = Wtf.get(this.body.child('.'+this.shimCls));
    },
    ...
    afterRender : function(container){
        var html = this.html;
        delete this.html;
        Wtf.ux.ManagedIframePanel.superclass.afterRender.call(this);
        if(this.iframe){
            if(this.defaultSrc){
                this.setSrc();
            }
            else if(html){
                this.iframe.update(typeof html == 'object' ? Wtf.DomHelper.markup(html) : html);
            }
        }
    ...
    ,on : function(name){
           ...
           Wtf.ux.ManagedIframePanel.superclass.on.apply(this, arguments);
    ...
    */
    setSrc : function(url, discardUrl,callback){
         ...
         var src = url || (Wtf.isIE&&Wtf.isSecure?Wtf.SSL_SECURE_URL:'');
    ...
    getState: function(){
         ...
         return Wtf.apply(Wtf.ux.ManagedIframePanel.superclass.getState.call(this) || {},
             URI?{defaultSrc  : typeof URI == 'function'?URI():URI}:null );

    },
    /**
     * Get the {@link Wtf.Updater} for this panel's iframe/or body. Enables you to perform Ajax-based document replacement of this panel's iframe document.
     * @return {Wtf.Updater} The Updater
     */
    getUpdater : function(){
        return this.rendered?(this.iframe||this.body).getUpdater():null;
    },
    /**
     * Get the embedded iframe Wtf.Element for this panel
     * @return {Wtf.Element} The Panels ux.ManagedIFrame instance.
     */
    getFrame : function(){
    ...
    /**
     * Get the embedded iframe's document as an Wtf.Element.
     * @return {Wtf.Element object} or null if unavailable
     */
...
Wtf.reg('iframepanel', Wtf.ux.ManagedIframePanel);

Wtf.ux.ManagedIframePortlet = Wtf.extend(Wtf.ux.ManagedIframePanel, {
     anchor: '100%',
     frame:true,
     collapseEl:'bwrap',
     collapsible:true,
...
Wtf.reg('iframeportlet', Wtf.ux.ManagedIframePortlet);

/* override adds a third visibility feature to Wtf.Element:
...
Wtf.apply(Wtf.Element.prototype, {
  setVisible : function(visible, animate){
        if(!animate || !Wtf.lib.Anim){
            if(this.visibilityMode == Wtf.Element.DISPLAY){
                this.setDisplayed(visible);
            }else if(this.visibilityMode == Wtf.Element.VISIBILITY){
                this.fixDisplay();
            ...
            this.anim({opacity: { to: (visible?1:0) }},
                  ...
                  null, .35, 'easeIn', function(){
                     ...
                     if(!visible){
                         if(visMode == Wtf.Element.DISPLAY){
                             dom.style.display = "none";
                         }else if(visMode == Wtf.Element.VISIBILITY){
                             dom.style.visibility = "hidden";
                         }else {
                             Wtf.get(dom).addClass(String(visMode));
                         }
