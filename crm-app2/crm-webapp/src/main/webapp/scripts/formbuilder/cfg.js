...
,repeat:{type:"Boolean/Object",desc:"True to repeat fire the click event while the mouse is down. This can also be an {@link Wtf.util.ClickRepeater} config object (defaults to false). @constructor Create a new button @param {Object} config The config object"}
...
,menuAlign:{type:"String",desc:"The position to align the menu to (see {@link Wtf.Element#alignTo} for more details, defaults to 'tl-bl?')."}
...
,template:{type:"Wtf.Template",desc:"(Optional) An {@link Wtf.Template} with which to create the Button's main element. This Template must contain numeric substitution parameter 0 if it is to display the text property. Changing the template could require code modifications if required elements (e.g. a button) aren't present."}
},
...
,style:{type:"String",desc:"A custom style specification to be applied to this component's Element. Should be a valid argument to {@link Wtf.Element#applyStyles}."}
...
,plugins:{type:"Object/Array",desc:"An object or array of objects that will provide custom functionality for this component. The only requirement for a valid plugin is that it contain an init method that accepts a reference of type Wtf.Component. When a component is created, if any plugins are available, the component will call the init method on each plugin, passing a reference to itself. Each plugin can then call methods or respond to events on the component as needed to provide its functionality."}
...
,layout:{type:"String",desc:"The layout type to be used in this container. If not specified, a default {@link Wtf.layout.ContainerLayout} will be created and used. Valid values are: accordion, anchor, border, card, column, fit, form and table. Specific config values for the chosen layout type can be specified using {@link #layoutConfig}."}
,layoutConfig:{type:"Object",desc:"This is a config object containing properties specific to the chosen layout (to be used in conjunction with the {@link #layout} config value). For complete details regarding the valid config options for each layout type, see the layout class corresponding to the type specified: {@link Wtf.layout.Accordion}, {@link Wtf.layout.AnchorLayout}, {@link Wtf.layout.BorderLayout}, {@link Wtf.layout.CardLayout}, {@link Wtf.layout.ColumnLayout}, {@link Wtf.layout.FitLayout}, {@link Wtf.layout.FormLayout} and {@link Wtf.layout.TableLayout}."}
,activeItem:{type:"String/Number",desc:"A string component id or the numeric index of the component that should be initially activated within the container's layout on render. For example, activeItem: 'item-1' or activeItem: 0 (index 0 = the first item in the container's collection). activeItem only applies to layout styles that can display items one at a time (like {@link Wtf.layout.Accordion}, {@link Wtf.layout.CardLayout} and {@link Wtf.layout.FitLayout}). Related to {@link Wtf.layout.ContainerLayout#activeItem}."}
,items:{type:"Mixed",desc:"A single item, or an array of items to be added to this container. Each item can be any type of object based on {@link Wtf.Component}, or a valid config object for such an item. If a single item is being passed, it should be passed directly as an object reference (e.g., items: {...}). Multiple items should be passed as an array of objects (e.g., items: [{...}, {...}])."}
,defaults:{type:"Object",desc:"A config object that will be applied to all components added to this container either via the {@link #items} config or via the {@link #add} or {@link #insert} methods. The defaults config can contain any number of name/value property pairs to be added to each item, and should be valid for the types of items being added to the container. For example, to automatically apply padding to the body of each of a set of contained {@link Wtf.Panel} items, you could pass: defaults: {bodyStyle:'padding:15px'}."}
...
,defaultType:{type:"String",desc:"The default type of container represented by this object as registered in {@link Wtf.ComponentMgr} (defaults to 'panel')."}
},


"CycleButton.js":{
items:{type:"Array",desc:"An array of {@link Wtf.menu.CheckItem} <b>config</b> objects to be used when creating the button's menu items (e.g., {text:'Foo', iconCls:'foo-icon'})"}
...
,changeHandler:{type:"Function",desc:"A callback function that will be invoked each time the active menu item in the button's menu has changed. If this callback is not supplied, the SplitButton will instead fire the {@link #change} event on active item change. The changeHandler function will be called with the following argument list: (SplitButton this, Wtf.menu.CheckItem item)"}
},


"DataView.js":{
tpl:{type:"String/Array",desc:"The HTML fragment or an array of fragments that will make up the template used by this DataView. This should be specified in the same format expected by the constructor of {@link Wtf.XTemplate}."}
,store:{type:"Wtf.data.Store",desc:"The {@link Wtf.data.Store} to bind this DataView to."}
,itemSelector:{type:"String",desc:"A CSS selector in any format supported by {@link Wtf.DomQuery} that will be used to filter the data loaded from the store (defaults to '')."}
...
,alignment:{type:"String",desc:"The position to align to (see {@link Wtf.Element#alignTo} for more details, defaults to \"c-c?\")."}
...
,method:{type:"String",desc:"The HTTP method to use to access the requested URL. Defaults to the {@link Wtf.form.Form}'s method, or if that is not specified, the underlying DOM form's method."}
,params:{type:"Mixed",desc:"Wtfra parameter values to pass. These are added to the Form's {@link Wtf.form.BasicForm#baseParams} and passed to the specified URL along with the Form's input fields."}
,success:{type:"Function",desc:"The function to call when a valid success return packet is recieved. The function is passed the following parameters: <ul> <li><code>form</code> : Wtf.form.Form<div class=\"sub-desc\">The form that requested the action</div></li> <li><code>action</code> : Wtf.form.Action<div class=\"sub-desc\">The Action class. The {@link #result} property of this object may be examined to perform custom postprocessing.</div></li> </ul>"}
,failure:{type:"Function",desc:"The function to call when a failure packet was recieved, or when an error ocurred in the Ajax communication. The function is passed the following parameters: <ul> <li><code>form</code> : Wtf.form.Form<div class=\"sub-desc\">The form that requested the action</div></li> <li><code>action</code> : Wtf.form.Action<div class=\"sub-desc\">The Action class. If an Ajax error ocurred, the failure type will be in {@link #failureType}. The {@link #result} property of this object may be examined to perform custom postprocessing.</div></li> </ul> / "}
,waitMsg:{type:"String",desc:"The message to be displayed by a call to {@link Wtf.MessageBox#wait} during the time the action is being processed."}
,waitTitle:{type:"String",desc:"The title to be displayed by a call to {@link Wtf.MessageBox#wait} during the time the action is being processed."}
...
,reader:{type:"DataReader",desc:"An Wtf.data.DataReader (e.g. {@link Wtf.data.XmlReader}) to be used to read data when executing \"load\" actions. This is optional as there is built-in support for processing JSON."}
,errorReader:{type:"DataReader",desc:"An Wtf.data.DataReader (e.g. {@link Wtf.data.XmlReader}) to be used to read data when reading validation errors on \"submit\" actions. This is completely optional as there is built-in support for processing JSON."}
...
,listener: {type:"String",desc:"Add different listeners to the events", events: [["Blur", "blur"],["Select", "select"],["Change", "change"],["Render", "render"],["Validateedit", "validateedit"]]}
},


"form/Combo.js":{
transform:{type:"Mixed",desc:"The id, DOM node or element of an existing select to convert to a ComboBox"}
,lazyRender:{type:"Boolean",desc:"True to prevent the ComboBox from rendering until requested (should always be used when rendering into an Wtf.Editor, defaults to false)"}
...
,store:{type:"Wtf.data.Store",desc:"The data store to which this combo is bound (defaults to undefined)"}
...
,listAlign:{type:"String",desc:"A valid anchor position value. See {@link Wtf.Element#alignTo} for details on supported anchor positions (defaults to 'tl-bl')"}
...
,listener: {type:"String",desc:"Add different listeners to the events", events: [["Blur", "blur"],["Select", "select"],["Change", "change"]]}
},
...
,layout:{type:"String",desc:"The {@link Wtf.Container#layout} to use inside the fieldset (defaults to 'form')."}
},
...
,vtype:{type:"String",desc:"A validation type name as defined in {@link Wtf.form.VTypes} (defaults to null)"}
...
,listener: {type:"String",desc:"Add different listeners to the events", events: [["Blur", "blur"],["Select", "select"],["Change", "change"]]}
,validator: {type:"String",desc:"Add validator function to the textfield"}
},
...
"grid/ColumnModel.js":{
id:{type:"String",desc:"(Optional) Defaults to the column's initial ordinal position. A name which identifies this column. The id is used to create a CSS class which is applied to all table cells in that column of the form <pre>x-grid-td-<b>id<b></pre> <br><br> The {@link Wtf.grid.Grid#autoExpandColumn} grid config option references the column via this identifier."}
,header:{type:"String",desc:"The header text to display in the Grid view."}
,dataIndex:{type:"String",desc:"(Optional) The name of the field in the grid's {@link Wtf.data.Store}'s {@link Wtf.data.Record} definition from which to draw the column's value. If not specified, the column's index is used as an index into the Record's data Array."}
,width:{type:"Number",desc:"(Optional) The initial width in pixels of the column. Using this instead of {@link Wtf.grid.Grid#autoSizeColumns} is more efficient."}
,sortable:{type:"Boolean",desc:"(Optional) True if sorting is to be allowed on this column. Defaults to the value of the {@link #defaultSortable} property. Whether local/remote sorting is used is specified in {@link Wtf.data.Store#remoteSort}."}
...
,store:{type:"Store",desc:"The Wtf.data.Store the grid should use as it's data source"}
,cm:{type:"Store",desc:"An Wtf.grid.ColumnModel for this grid"}
...
,view:{type:"Object",desc:"The {@link Wtf.grid.GridView} used by the grid. This can be set before a call to render()."}
,loadMask:{type:"Object",desc:"An {@link Wtf.LoadMask} config or true to mask the grid while loading (defaults to false)."}
...
,split:{type:"Boolean",desc:"True to display a {@link Wtf.SplitBar} between this region and its neighbor, allowing the user to resize the regions dynamically (defaults to false). When split = true, it is common to specify a {@link #minSize} and {@link #maxSize} for the region."}
...
"menu/Item.js":{
icon:{type:"String",desc:"The path to an icon to display in this menu item (defaults to Wtf.BLANK_IMAGE_URL)"}
...
,subMenuAlign:{type:"String",desc:"The {@link Wtf.Element#alignTo} anchor position value to use for submenus of this menu (defaults to \"tl-tr?\")"}
,defaultAlign:{type:"String",desc:"The default {@link Wtf.Element#alignTo) anchor position value for this menu relative to its element of origin (defaults to \"tl-bl?\")"}
...
"Panel.js":{
tbar:{type:"Object/Array",desc:"The top toolbar of the panel. This can be a {@link Wtf.Toolbar} object, a toolbar config, or an array of buttons/button configs to be added to the toolbar. Note that this is not available as a property after render. To access the top toolbar after render, use {@link #getTopToolbar}."}
,bbar:{type:"Object/Array",desc:"The bottom toolbar of the panel. This can be a {@link Wtf.Toolbar} object, a toolbar config, or an array of buttons/button configs to be added to the toolbar. Note that this is not available as a property after render. To access the bottom toolbar after render, use {@link #getBottomToolbar}."}
...
,buttons:{type:"Array",desc:"An array of {@link Wtf.Button} instances (or valid button configs) to add to the footer of this panel"}
,autoLoad:{type:"Object/String/Function",desc:"A valid url spec according to the Updater {@link Wtf.Updater#update} method. If autoLoad is not null, the panel will attempt to laod its contents immediately upon render."}
...
,bodyStyle:{type:"String",desc:"Custom CSS styles to be applied to the body element in the format expected by {@link Wtf.Element#applyStyles} (defaults to null)."}
...
,shadow:{type:"Boolean/String",desc:"True (or a valid Wtf.Shadow {@link Wtf.Shadow#mode} value) to display a shadow behind the panel, false to display no shadow (defaults to 'sides'). Note that this option opnly applies when floating = true."}
...
,keys:{type:"Object/Array",desc:"A KeyMap config object (in the format expected by {@link Wtf.KeyMap#addBinding) used to assign custom key handling to this panel (defaults to null)."}
...
,animCollapse:{type:"Boolean",desc:"True to animate the transition when the panel is collapsed, false to skip the animation (defaults to true if the {@link Wtf.Fx} class is available, otherwise false)."}
...
,shadow:{type:"Boolean/String",desc:"True or \"sides\" for the default effect, \"frame\" for 4-way shadow, and \"drop\" for bottom-right shadow (defaults to \"sides\")"}
,defaultAlign:{type:"String",desc:"The default {@link Wtf.Element#alignTo) anchor position value for this tip relative to its element of origin (defaults to \"tl-bl?\")"}
},
...
"tree/TreeEditor.js":{
alignment:{type:"String",desc:"The position to align to (see {@link Wtf.Element#alignTo} for more details, defaults to \"l-l\")."}
...
,uiProviders:{type:"Object",desc:"(optional) An object containing properties which specify custom {@link Wtf.tree.TreeNodeUI} implementations. If the optional <i>uiProvider</i> attribute of a returned child node is a string rather than a reference to a TreeNodeUI implementation, this that string value is used as a property name in the uiProviders object."}
...
,qtip:{type:"String",desc:"An Wtf QuickTip for the node"}
,expandable:{type:"Boolean",desc:"If set to true, the node will always show a plus/minus icon, even when empty"}
,qtipCfg:{type:"String",desc:"An Wtf QuickTip config for the node (used instead of qtip)"}
,singleClickExpand:{type:"Boolean",desc:"True for single click expand on this node"}
,uiProvider:{type:"Function",desc:"A UI <b>class</b> to use for this node (defaults to Wtf.tree.TreeNodeUI)"}
...
,dragConfig:{type:"Object",desc:"Custom config to pass to the {@link Wtf.tree.TreeDragZone} instance"}
,dropConfig:{type:"Object",desc:"Custom config to pass to the {@link Wtf.tree.TreeDropZone} instance"}
...
,hlDrop:{type:"Boolean",desc:"false to disable node highlight on drop (defaults to the value of Wtf.enableFx)"}
,hlColor:{type:"String",desc:"The color of the node highlight (defaults to C3DAF9)"}
,animate:{type:"Boolean",desc:"true to enable animated expand/collapse (defaults to the value of Wtf.enableFx)"}
,singleExpand:{type:"Boolean",desc:"true if only 1 node per branch may be expanded"}
,selModel:{type:"Boolean",desc:"A tree selection model to use with this TreePanel (defaults to a {@link Wtf.tree.DefaultSelectionModel})"}
...
,resizeHandles:{type:"String",desc:"A valid {@link Wtf.Resizable} handles config string (defaults to 'all'). Only applies when resizable = true."}
,manager:{type:"Wtf.WindowGroup",desc:"A reference to the WindowGroup that should manage this window (defaults to {@link Wtf.WindowMgr})."}
...
"-":{
	hideLabel:{type:"Boolean",desc:"True to hide labels (applies to subpanels, defaults to false"}
}
