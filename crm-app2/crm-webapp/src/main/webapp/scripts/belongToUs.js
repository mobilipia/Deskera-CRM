...
*/
var LOGIN_PREFIX = 'Common/AuthHandler/verifyLogin.do';
var CASELOGIN_PREFIX = 'Common/AuthHandler/verifyCaseLogin.do';
...
function validateLogin(u, p, isMobileRequest,isCaseRequest){
    var d = new Date().getTime();
    var p = 't=a&x=' + encodeURI(hex_hmac_sha1(d, p) + '&u=' + encodeURI(u) + '&p=' + encodeURI(hex_sha1(p)) + '&dc=' + encodeURI(d));
    var useragent = navigator.userAgent;
//    var isBlackberry = !!useragent.match(/blackberry/i);
    if(isCaseRequest){
    	http.open('POST', CASELOGIN_PREFIX, true);
    }else{
    ...
    if(isMobileRequest)
        http.onreadystatechange = handleMobileValidateLogin;
    if(isCaseRequest)
    	http.onreadystatechange = handleCaseValidateLogin;
    else
...
function handleCaseValidateLogin() {
    if (http.readyState == NORMAL_STATE) {
       if (http.responseText && http.responseText.length > 0) {
            ...
            if (results.data.success == true) {
                var u = "./crm/Case/customeraction/getCustomerCases.do";
                redirect(u);
            }
            else {
                setMsg(results.data.message, 0);
            }
        }
        else {
...
function handleMobileValidateLogin() {
    if (http.readyState == NORMAL_STATE) {
       if (http.responseText && http.responseText.length > 0) {
            var results = eval("(" + trimStr(http.responseText) + ")");
            if (results.data.success == true) {
                var u = "./bbmobile/welcomepage.jsp";
                redirect(u);
            }
            else {
                setMsg(results.data.message, 0);
...
function handleHttpValidateLogin(){
    if (http.readyState == NORMAL_STATE) {
       if (http.responseText && http.responseText.length > 0) {
            var results = eval("(" + trimStr(http.responseText) + ")");
            if (results.data.success == true) {
                SetCookie("lid", results.data.lid);
                SetCookie("username", results.data.username);
                SetCookie("cid",results.data.companyid);
...
function lValidateEmpty(isMobileRequest){
    var usr = $('#UserName')[0];
    var pwd = $('#Password')[0];
    ...
    if (!(bL || bP)) {
        validateLogin(trimStr(usr.value), pwd.value,isMobileRequest);
    }
}

function checkEmpty(isCaseRequest){
    var usr = $('#UserName')[0];
    var pwd = $('#Password')[0];
    var usrReq = $('#UserNameRequired')[0];
    var pwdReq = $('#PasswordRequired')[0];
    ...
    setVisibility(bP, pwdReq);
    if (!(bL || bP)) {
        validateLogin(trimStr(usr.value), pwd.value,undefined,isCaseRequest);
    }
}
            ...
            if (res != "" && res != null) {
                ...
                if (resObj.data =="msg:{succcess: true}") {
                    ...
                    if (resObj.error =="msg:{companyname failure}") {
                        msg = " Company Name already exits ";
                    }
...
//        msg += "Enter your company domain<br>";
   ...
   //     signUpUser(trimStr(usr.value), p1.value, trimStr(eml.value), trimStr(cname.value), trimStr(cdomain.value), trimStr(fname.value), trimStr(address.value));//,trimStr(city.value),trimStr(zipcode.value),trimStr(country.value));
        signUpUser(trimStr(usr.value), p1.value, trimStr(eml.value), trimStr(cname.value), "demo", trimStr(fname.value), trimStr(address.value));//,trimStr(city.value),trimStr(zipcode.value),trimStr(country.value));
   clearErrorMsg();
 }
    else {
