...
Wtf.extend(Wtf.common.pPageSize, Wtf.util.Observable, {
     ...
     */
    beforeText: WtfGlobal.getLocaleText("crm.paging.show"),//'Show',
    
    /**
     * @cfg {String} afterText
     * Text to display after the comboBox
     */
    afterText: WtfGlobal.getLocaleText("crm.paging.items"),//'items',
    ...
    fixvariations: [[5], [10], [15], [25], [35], [50], [60], [70], [80], [90], [100]],
    
    init: function(pagingToolbar){
        if(this.recordsLimit && Wtf.isChrome)
    ...
    addToStore: function(value){
        if (value > 0) {
            if(this.spreadSheet && this.recordsLimit) {
    ...
    changePageSize: function(value){
        ...
        if((!this.spreadSheet && !this.recordsLimit) || Wtf.isChrome)
            this.updateStore();
        this.combo.collapse();
    },

    updatePagingMsg:function(value, totalCount) {
        var pt = this.pagingToolbar;
        var totalPages;
        if(pt.displayEl){
            var msg = value == 0 ?
                ...
                String.format(
                    pt.displayMsg,
                    pt.cursor+1, pt.cursor+value, totalCount
                );
            pt.displayEl.update(msg);
        }
        totalPages = totalCount < pt.pageSize ? 1 : Math.ceil(totalCount/pt.pageSize)
        pt.afterTextEl.el.innerHTML = String.format(pt.afterPageText, totalPages);
        var ap = pt.getPageData().activePage;
        pt.next.setDisabled(ap == totalPages);
        pt.last.setDisabled(ap == totalPages);
    },
