        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.summaryType) {
                for (var j = 0, jlen = rs.length; j < jlen; j++) {
                    ...
                    if(cf.summaryType=='min' && j == 0){
                        data[cname] = Wtf.getValidNumberOrDefault(r.get(cname), 0);
                    }
                    if(r.get(cname) != "" && r.get(cname)!=undefined){
                    	data[cname] = Wtf.ux.grid.GridSummary.Calculations[cf.summaryType](r.get(cname), r, cname, data, cnt);
                    	cnt = data[cname];
                    }
                }
                data[cname] = "<div style='font-size:14px;font-weight:bold;'>"+cf.summaryType + " : " + data[cname]+"</div>";
            }
        }
