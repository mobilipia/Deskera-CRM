...
Wtf.lib.Ajax.combinereq = function(){
    ...
    while(this._queue.length>0){
        ...
        if(ele.uri=="crm/Dashboard/General/getData.do"){
            newparam.push(ele);
            ele = null;
            continue
        }
        ...
        if(typeof(ele.callback.argument.options.params)=="string")
            ele.callback.argument.options.params = Wtf.urlDecode(ele.callback.argument.options.params,true);
    ...
    if(param.length>0){
        if(ele){
            if(this._activeRequests < this.Queue_Size){
                this._asyncRequest(ele.o, ele.method, "crm/Dashboard/General/getData.do", ele.callback, Wtf.urlEncode(param));
            }else{
                this._queue.push({
                   o : ele.o,
                   method: ele.method,
                   uri: "crm/Dashboard/General/getData.do",
...
Wtf.lib.Ajax._processQueue = function(flag)
{
    ...
    if(flag==1){
        to = this._queue[0];
        if(to&&to.uri=="crm/Dashboard/General/getData.do"){
            this.simplerequest();
            return;
        }
    }else{
...
Wtf.Ajax.requestEx = function(config, scope, successCallback, failureCallback){
    ...
    for (var key in config.params) {
      if (config.params.hasOwnProperty(key)) {
          var attObj = config.params[key];
          newConfig[key] = WtfGlobal.removeNL(attObj);
      }
    }
    if(config.params)
        config.params['randomnumber'] = random_number;
    Wtf.Ajax.request({
        method: "POST",
        url: config.url,
        scope: scope,
...
function clog(e){
	var cons = (Wtf.isIE)?window.console:console;
	if(cons!=undefined){
		if(cons && cons.debug && e){
			cons.debug(e.toString());
		}
	}
}
