...
Wtf.extend(Wtf.attributeComponent, Wtf.Panel, {
    ...
    onRender: function(config){
        ...
        this.count = 0;
        Wtf.Ajax.requestEx({
            url: "Common/CRMCommon/getconfigdata.do",
            method: 'POST',
            params: {
                configFlag : "true",
        ...
        function(responseObj) {
            ...
            if(responseObj.data !='' && responseObj.data !=null){
                ...
                for(var i = 0; i < responseObj.data.length; i++) {
                    ...
                    if(responseObj.data[i].configtype=="1"){   // TextField
                    ...
                    } else if(responseObj.data[i].configtype=="2"){  // NumberField

                        this.addNumberField(responseObj.data[i]);

                    } else if(responseObj.data[i].configtype=="3"){  // DateField

                        this.addDateField(responseObj.data[i]);

                    } else if(responseObj.data[i].configtype=="4" || responseObj.data[i].configtype=="7") {  // ComboBox Or Multi-Select Combo Box
                    ...
                    } else if(responseObj.data[i].configtype=="5"){  // TimeField

                        this.addTimeField(responseObj.data[i]);

                    } else if(responseObj.data[i].configtype=="6"){ // CheckBox

                        this.addCheckBox(responseObj.data[i]);

                    } else if(responseObj.data[i].configtype=="8"){ // Reference Module
...
//    validSave:function(fieldobj,newval,oldval){
//        var field = fieldobj.recordname
//        var sdate = Wtf.getCmp(this.sdate+this.id).getValue();
//        var edate = Wtf.getCmp(this.edate+this.id).getValue();
//
//
//        if(field.replace(" ","").toLowerCase()=="startdate" || field.replace(" ","").toLowerCase()=="enddate") {
//            if(field.replace(" ","").toLowerCase()=="enddate" ){
//                if(sdate > edate) {
//                    Wtf.getCmp(this.sdate+this.id).setValue(edate);
//                }
//            }
//            if(field.replace(" ","").toLowerCase()=="startdate" ){
//                if(sdate > edate) {
//                    Wtf.getCmp(this.edate+this.id).setValue(sdate);
//                }
//            }
//        }
//
//    },
//    afterchange: function(field,newval,oldval){
//        this.validSave(field,newval,oldval);
//
//    },
    // Kuldeep Singh : Add TextField
...
//        if(this.sdate!="0"&&this.edate!="0"){
//            Wtf.getCmp(this.sdate+this.id).on('change',this.afterchange,this);
//            Wtf.getCmp(this.edate+this.id).on('blur',this.afterchange,this);
//        }    
    },
    ...
    addComboBox : function(responseObjData){
        ...
        if(responseObjData.configtype=="4") {
            this.ruleTypeCombo = new Wtf.form.ComboBox({
                ...
                emptyText: (createStore ? 'Select': 'Search')+' a record',
                allowBlank:responseObjData.allowblank,
        ...
        } else if(responseObjData.configtype==7) {
