...
public class ContactEventService implements ModuleEventService{
	...
	public List<CalendarEvent> getEvents(StringBuffer userList,HashMap<String,Object> params) {
		ArrayList filter_names = new ArrayList((List<String>) Arrays.asList("c.crmContact.deleteflag", "c.crmContact.validflag","c.crmActivityMaster.deleteflag", "c.crmActivityMaster.validflag"));
		...
		try {
			rows = (List <Object[]>)(activityDao.getContactActivity(requestParams, userList, filter_names, filter_params).getEntityList());
		} catch (ServiceException e) {}
		List<CalendarEvent> events = new ArrayList<CalendarEvent>();
		if(rows!=null){
			for(Object[] row:rows){
				events.add(prepareEvent((CrmContact)row[1], (CrmActivityMaster)row[0]));
			}
		}
		return events;
	}
	
	private CalendarEvent prepareEvent(CrmContact contact,CrmActivityMaster activity){
		CalendarEvent event = new CalendarEvent();
		event.setId(activity.getActivityid());
		String flag = getString(activity.getFlag(), "Event");
		String subject = getString(contact.getFullname(),"")+ " Contact's "+flag;
