...
public class AccountEventService implements ModuleEventService {
	...
	public List<CalendarEvent> getEvents(StringBuffer userList,HashMap<String,Object> params) {
        ArrayList filter_names = new ArrayList((List<String>) Arrays.asList("c.crmAccount.deleteflag", "c.crmAccount.validflag","c.crmActivityMaster.deleteflag", "c.crmActivityMaster.validflag"));
		...
		try {
			rows = (List <Object[]>)(activityDao.getAccountActivity(requestParams, userList, filter_names, filter_params).getEntityList());
		} catch (ServiceException e) {}
		List<CalendarEvent> events = new ArrayList<CalendarEvent>();
		if(rows!=null){
			for(Object[] row:rows){
				events.add(prepareEvent((CrmAccount)row[1], (CrmActivityMaster)row[0]));
			}
		}
		return events;
	}
	
	private CalendarEvent prepareEvent(CrmAccount account,CrmActivityMaster activity){
		CalendarEvent event = new CalendarEvent();
		event.setId(activity.getActivityid());
		String flag = getString(activity.getFlag(), "Event");		
		String subject = getString(account.getAccountname(),"")+ " Account's "+flag;
